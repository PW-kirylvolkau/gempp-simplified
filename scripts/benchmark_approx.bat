@echo off
REM Benchmark script for gempp (Windows) using STSM upper-bound approximation
REM Usage: benchmark_approx.bat [upperbound]
REM   upperbound: optional, in (0,1], default 0.5. Must be the only argument.

setlocal enabledelayedexpansion

if not "%~2"=="" (
    echo Usage: %~nx0 [upperbound]
    exit /b 1
)

set UPPERBOUND=0.5
if not "%~1"=="" (
    set UPPERBOUND=%~1
)

set SCRIPT_DIR=%~dp0
set PROJECT_DIR=%SCRIPT_DIR%..
set BUILD_DIR=%PROJECT_DIR%\build
set BENCHMARKS_DIR=%PROJECT_DIR%\benchmarks
set RESULTS_FILE=%BENCHMARKS_DIR%\results_approx.csv

echo === Building gempp ===
if not exist "%BUILD_DIR%" mkdir "%BUILD_DIR%"
cd /d "%BUILD_DIR%"

cmake .. >nul
if errorlevel 1 (
    echo CMake configuration failed
    exit /b 1
)

cmake --build . --config Release >nul
if errorlevel 1 (
    echo Build failed
    exit /b 1
)

cd /d "%PROJECT_DIR%"

set EXE=%PROJECT_DIR%\gempp.exe

if not exist "%EXE%" (
    echo Build failed: executable not found at %EXE%
    exit /b 1
)

if not exist "%BENCHMARKS_DIR%" mkdir "%BENCHMARKS_DIR%"

echo === Running approximation benchmarks (upperbound=%UPPERBOUND%) ===
echo Test,Upperbound,Time (ms),GED > "%RESULTS_FILE%"

echo --- Running on existing tests ---
for /d %%D in ("%PROJECT_DIR%\tests\*") do (
    set TEST_NAME=%%~nxD
    set INPUT=%%D\input.txt
    set TIME_MS=
    set GED_VAL=
    if exist "!INPUT!" (
        for /f "tokens=*" %%O in ('"%EXE%" --approx-stsm --upperbound %UPPERBOUND% --time "!INPUT!" 2^>^&1') do (
            echo %%O | findstr /C:"Time:" >nul && (
                for /f "tokens=2" %%T in ("%%O") do set TIME_MS=%%T
            )
            echo %%O | findstr /C:"GED:" >nul && (
                for /f "tokens=2" %%G in ("%%O") do set GED_VAL=%%G
            )
        )
        echo !TEST_NAME!: !TIME_MS!ms, GED=!GED_VAL!
        echo !TEST_NAME!,%UPPERBOUND%,!TIME_MS!,!GED_VAL! >> "%RESULTS_FILE%"
    )
)

echo.
echo === Benchmark complete ===
echo Upperbound used: %UPPERBOUND%
echo Results saved to: %RESULTS_FILE%
pause


